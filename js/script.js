...
async function fetchPrices() {
    ...
    for (let i = 0; i < results.length; i++) {
        ...
        if (precision < -1) {
            price = Number(Math.round(price * Math.pow(10, -precision)) * Math.pow(10, precision)).toFixed(-precision);
        } else {
...
async function acquire(tokenId) {
    ...
    try {
        const price = (await contract.acquisitionPrice(tokenId));
        const contractWithSigner = contract.connect(provider.getSigner(address));
        const tx = await contractWithSigner.acquire(tokenId, price, { value: price });
        await tx.wait();
    } catch(err) {
        alert(err.message);
    }
